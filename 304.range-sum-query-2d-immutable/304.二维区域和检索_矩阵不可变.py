...
class NumMatrix:
    ...
    def __init__(self, matrix: List[List[int]]):
        ...
        # 这里 preSum 比原矩阵 matrix 多了一行一列，是为了让第 0 行与第 0 列的元素也能使用上面的递推公式
        self.preSum = [[0] * (n + 1) for _ in range(m + 1)]

        for i in range(1, m + 1):
            for j in range(1, n + 1):
                # 因为 preSum 比 matrix 多了一行一列，因此加的 matrix 对应的值应该是 matrix[i-1][j-1]
                self.preSum[i][j] = self.preSum[i-1][j] + self.preSum[i][j-1] - self.preSum[i-1][j-1] + matrix[i-1][j-1]

    def sumRegion(self, row1: int, col1: int, row2: int, col2: int) -> int:
        # 同样 preSum 比 matrix 多了一行一列，所有的下标都加了1
